...
INPUT_CSV    = DATA_DIR / "state_forms_by_class.csv"
OUT_CSV      = DATA_DIR / "state_successors.csv"
DONE_FILE    = DATA_DIR / "state_successors.done"

SPARQL_ENDPOINT = "https://query.wikidata.org/sparql"
USER_AGENT      = "GeoMundi-Successors/1.1 (+cfmessias@gmail.com)"
REQUEST_TIMEOUT = 90
RETRY_MAX       = 4
BACKOFF_BASE_S  = 6
THROTTLE_S      = 0.8
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Sucessores (P1366/P156) — incremental, filtrado ao universo CLS_STATE_LIKE.")
    ...
    for i, (qid, lbl_hint, iso_hint) in enumerate(to_process, 1):
        print(f"[{i}/{len(to_process)}] next {qid}…", flush=True)
        try:
            js = run_sparql(q_next(qid))
            rows = parse(js)
            filtered = []
    ...
    print(f"✔️ successors: novas linhas {new_total} -> {OUT_CSV}")
