...
def _chart_one(df: pd.DataFrame, code: str, label: str) -> alt.Chart:
    ...
    return (
        ...
        .encode(
            x=alt.X("year:Q", title=tr("economics.chart_tooltip.year"), axis=alt.Axis(format="d")),
            y=alt.Y("value:Q", title=None, scale=alt.Scale(zero=False), axis=alt.Axis(format=y_fmt)),
            tooltip=[
...
def _donut_fig(labels: list[str],
               values: list[float],
               title: str,
    ...
    # Se recebemos uma ordem de categorias, reordenamos os arrays
    if category_order:
    ...
    # cores na mesma ordem dos labels
    if color_map:
    ...
    fig = go.Figure(
        go.Pie(
            ...
            values=values,
            hole=0.55,
            sort=False,                 # mantém a ordem dada
...
def render_sectors_panel(iso3: str):
    ...
    if view == tr("economics.presets.sectors_vab") and not data["vab_wide"].empty:
        ...
        # Ordem e paleta coerentes
        domain = order_vab[:]  # [Agr, Ind, Serv]
        rng    = [palette[d] for d in domain]
        # Fixar ordem categórica no dataframe (Altair respeita)
