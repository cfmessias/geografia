...
def _country_selector(countries_df: pd.DataFrame) -> tuple[str | None, str | None]:
    ...
    with st.form("pais_form", clear_on_submit=False):
        ...
        if not opts:
            ...
            st.form_submit_button("🔎 Abrir", width="stretch")  # mantém o layout da form
            return None, None
        ...
        with c2:
            submitted = st.form_submit_button("🔎 Abrir", width="stretch")
...
def render_paises_tab():
    ...
    with st.expander("Património Mundial (UNESCO)"):
        ...
        if not u.empty:
            ...
            st.dataframe(u[["Sítio","Tipo","Ano","lat","lon"]], width="stretch", hide_index=True)

            if {"lat","lon"}.issubset(u.columns):
                st.map(u[["lat","lon"]].dropna(), width="stretch")
        else:
            st.caption("—")
